...
const Page = () => {
  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const password = CryptoJS.SHA256(loginData.password).toString();
    console.log(loginData.email, password);
    try {
      const response = await fetch("api/login", {method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify(loginData)});
